...
- Inner sub-nodes go in `nodes:` (not `blocks:`)
- Inner edges go in the loop node's `edges:` array
- Entry edge: `source_port: loop-function-inline-output` (loop → first inner)
- Exit edge: `target_port: loop-function-inline-input` (last inner → loop)
- Top-level exit edge: `source_port: loop-output` (loop → next outer node)
- `canvas_position`: separate from `position` (for inner canvas layout)

See `examples/loop-workflow.yaml` for complete template.
