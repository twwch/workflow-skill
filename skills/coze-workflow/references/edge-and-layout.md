# Edge Connections and Layout Rules

## coze.cn Cloud YAML Edge Format
...
edges:
    ...
    - source_node: "200001"
      target_node: "200002"
      source_port: loop-output        # exit from loop
```

### Special ports (Loop nodes)

| Port | Where | Purpose |
|------|-------|---------|
| `loop-function-inline-output` | Inner edge `source_port` | Loop node → first inner node |
| `loop-function-inline-input` | Inner edge `target_port` | Last inner node → loop node |
| `loop-output` | Outer edge `source_port` | Loop node → next outer node |

### Layout coordinates

- Start node: `x: -1810, y: 0`
- Horizontal spacing: ~600px between nodes
- Parallel branches: offset ±200-400 on Y axis
- End node: rightmost position

---

## Open-Source Canvas JSON Edge Format
